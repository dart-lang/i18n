...
void runICUTests({
  ...
  if (!_setupICU(
    systemIcuVersion: systemIcuVersion,
    specialIcuLibPath: specialIcuLib,
  )) {
    return;
  }
...
void _validateShort(
  ...
  var skip = skipLocales.contains(locale)
      ? 'Skipping problem locale $locale'
      : false;
...
void _validateFancy(more_testdata.CompactRoundingTestCase t) {
  ...
  test(t.toString(), () {
    expect(
      _formatWithUnumf(locale, skel, t.number),
      t.expected,
...
bool _setupICU({int? systemIcuVersion, String? specialIcuLibPath}) {
  ...
  if (systemIcuVersion != null) {
    ...
    try {
      DynamicLibrary libicuuc = DynamicLibrary.open(
        'libicuuc.so.$systemIcuVersion',
      );
  ...
  unumf_openForSkeletonAndLocale = libicui18n
      .lookupFunction<
        NativeUnumfOpenForSkeletonAndLocaleOp,
        UnumfOpenForSkeletonAndLocaleOp
      >('unumf_openForSkeletonAndLocale$icuVersionSuffix');
  unumf_openResult = libicui18n
      .lookupFunction<NativeUnumfOpenResultOp, UnumfOpenResultOp>(
        'unumf_openResult$icuVersionSuffix',
      );
  unumf_formatDouble = libicui18n
      .lookupFunction<NativeUnumfFormatDoubleOp, UnumfFormatDoubleOp>(
        'unumf_formatDouble$icuVersionSuffix',
      );
  unumf_formatInt = libicui18n
      .lookupFunction<NativeUnumfFormatIntOp, UnumfFormatIntOp>(
        'unumf_formatInt$icuVersionSuffix',
      );
  unumf_resultToString = libicui18n
      .lookupFunction<NativeUnumfResultToStringOp, UnumfResultToStringOp>(
        'unumf_resultToString$icuVersionSuffix',
      );
  unumf_close = libicui18n.lookupFunction<NativeUnumfCloseOp, UnumfCloseOp>(
    'unumf_close$icuVersionSuffix',
  );
  unumf_closeResult = libicui18n
      .lookupFunction<NativeUnumfCloseResultOp, UnumfCloseResultOp>(
...
String _formatWithUnumf(String locale, String skeleton, num number) {
  ...
  cErrorCode.value = 0;
  final uformatter = unumf_openForSkeletonAndLocale!(
    cSkeleton,
    -1,
    cLocale,
    cErrorCode,
  ...
  expect(
    errorCode,
    equals(15), // U_BUFFER_OVERFLOW_ERROR
    reason: u_errorName!(errorCode).toString(),
  );
  cErrorCode.value = 0;
...
typedef NativeUnumfOpenResultOp =
    Pointer<UFormattedNumber> Function(Pointer<Int32> ec);

/// Dart signature for
/// [unumf_openResult()](http://icu-project.org/apiref/icu4c/unumberformatter_8h.html#a5bd2d297cb2664b4964d25fd41671dad)
typedef UnumfOpenResultOp =
    Pointer<UFormattedNumber> Function(Pointer<Int32> ec);
...
typedef NativeUnumfFormatIntOp =
    Void Function(
      Pointer<UNumberFormatter> uformatter,
      Int32 value,
      Pointer<UFormattedNumber> uresult,
...
typedef NativeUnumfResultToStringOp =
    Int32 Function(
      Pointer<UFormattedNumber> uresult,
