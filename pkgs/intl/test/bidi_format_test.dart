...
void main() {
  ...
  test('wrapWithSpan', () {
    ...
    expect(
      ltrFmt.wrapWithSpan('$longEn$he$html', isHtml: true, resetDir: true),
      equals('$longEn$he$html$LRM'),
    );
    // Test overall dir (but not exit dir) is opposite to context dir, dirReset.
    expect(
      rtlFmt.wrapWithSpan('$longEn$he', isHtml: true, resetDir: true),
      equals('<span dir=ltr>$longEn$he</span>$RLM'),
    );

    // Test input is plain text (not escaped).
    expect(
      ltrFmt.wrapWithSpan('<br>$en', isHtml: false, resetDir: false),
      equals('&lt;br&gt;$en'),
    );
    ...
    expect(
      ltrAlwaysSpanFmt.wrapWithSpan(en, isHtml: true, resetDir: false),
      equals('<span>$en</span>'),
    );
    // Test alwaysSpan, overall dir matches context dir (LTR), dirReset.
    expect(
      ltrAlwaysSpanFmt.wrapWithSpan(en, isHtml: true, resetDir: true),
      equals('<span>$en</span>'),
    );
    // Test alwaysSpan, overall dir matches context dir (RTL), no dirReset.
    expect(
      rtlAlwaysSpanFmt.wrapWithSpan(he, isHtml: true, resetDir: false),
      equals('<span>$he</span>'),
    );
    // Test alwaysSpan, overall dir matches context dir (RTL), dirReset.
    expect(
      rtlAlwaysSpanFmt.wrapWithSpan(he, isHtml: true, resetDir: true),
      equals('<span>$he</span>'),
    );
    ...
    // no dirReset.
    expect(
      ltrAlwaysSpanFmt.wrapWithSpan(he, isHtml: true, resetDir: false),
      equals('<span dir=rtl>$he</span>'),
    );
    // Test alwaysSpan, overall dir (RTL) doesn't match context dir (LTR),
    // dirReset.
    expect(
      ltrAlwaysSpanFmt.wrapWithSpan(he, isHtml: true, resetDir: true),
      equals('<span dir=rtl>$he</span>$LRM'),
    );
    // Test alwaysSpan, overall dir (neutral) doesn't match context dir (LTR),
    // dirReset
    expect(
      ltrAlwaysSpanFmt.wrapWithSpan('', isHtml: true, resetDir: true),
      equals('<span></span>'),
    );
    ...
    expect(
      ltrFmt.wrapWithSpan(he, direction: TextDirection.LTR),
      equals('$he$LRM'),
    );
  ...
  test('wrapWithUnicode', () {
    // Test overall dir matches context dir (LTR), no dirReset.
    expect(
      ltrFmt.wrapWithUnicode(en, isHtml: true, resetDir: false),
      equals(en),
    );
    // Test overall dir matches context dir (LTR), dirReset.
    expect(
      ltrFmt.wrapWithUnicode(en, isHtml: true, resetDir: true),
      equals(en),
    );
    // Test overall dir matches context dir (RTL), no dirReset.
    expect(
      rtlFmt.wrapWithUnicode(he, isHtml: true, resetDir: false),
      equals(he),
    );
    // Test overall dir matches context dir (RTL), dirReset.
    expect(
      rtlFmt.wrapWithUnicode(he, isHtml: true, resetDir: true),
      equals(he),
    );

    // Test overall dir (RTL) doesn't match context dir (LTR), no dirReset.
    expect(
      ltrFmt.wrapWithUnicode(he, isHtml: true, resetDir: false),
      equals('$RLE$he$PDF'),
    );
    // Test overall dir (RTL) doesn't match context dir (LTR), dirReset.
    expect(
      ltrFmt.wrapWithUnicode(he, isHtml: true, resetDir: true),
      equals('$RLE$he$PDF$LRM'),
    );
    // Test overall dir (LTR) doesn't match context dir (RTL), no dirReset.
    expect(
      rtlFmt.wrapWithUnicode(en, isHtml: true, resetDir: false),
      equals('$LRE$en$PDF'),
    );
    // Test overall dir (LTR) doesn't match context dir (RTL), dirReset.
    expect(
      rtlFmt.wrapWithUnicode(en, isHtml: true, resetDir: true),
      equals('$LRE$en$PDF$RLM'),
    );
    // Test overall dir (LTR) doesn't match context dir (unknown), no dirReset.
    expect(
      unkFmt.wrapWithUnicode(en, isHtml: true, resetDir: false),
      equals('$LRE$en$PDF'),
    );
    // Test overall dir (RTL) doesn't match context dir (unknown), dirReset.
    expect(
      unkFmt.wrapWithUnicode(he, isHtml: true, resetDir: true),
      equals('$RLE$he$PDF'),
    );
    // Test overall dir (neutral) doesn't match context dir (LTR), dirReset.
    expect(
      ltrFmt.wrapWithUnicode('', isHtml: true, resetDir: true),
      equals(''),
    );
    ...
    // Test overall dir (but not exit dir) is opposite to context dir, dirReset.
    expect(
      rtlFmt.wrapWithUnicode('$longEn$he', isHtml: true, resetDir: true),
      equals('$LRE$longEn$he$PDF$RLM'),
    );

    // Test overall dir matches context dir (LTR).
    expect(
      ltrFmt.wrapWithUnicode(en, direction: TextDirection.LTR),
      equals(en),
    );
    // Test overall dir (but not exit dir) supposedly matches context dir (LTR).
    expect(
      ltrFmt.wrapWithUnicode(he, direction: TextDirection.LTR),
      equals('$he$LRM'),
    );
    // Test overall dir matches context dir (RTL).
    expect(
      rtlFmt.wrapWithUnicode(he, direction: TextDirection.RTL),
      equals(he),
    );
    // Test overall dir (but not exit dir) supposedly matches context dir (RTL).
    expect(
      rtlFmt.wrapWithUnicode(en, direction: TextDirection.RTL),
      equals('$en$RLM'),
    );

    // Test overall dir (RTL) doesn't match context dir (LTR).
    expect(
      ltrFmt.wrapWithUnicode(he, direction: TextDirection.RTL),
      equals('$RLE$he$PDF$LRM'),
    );
    // Test supposed overall dir (RTL) doesn't match context dir (LTR).
    expect(
      ltrFmt.wrapWithUnicode(en, direction: TextDirection.RTL),
      equals('$RLE$en$PDF$LRM'),
    );
    // Test overall dir (LTR) doesn't match context dir (RTL).
    expect(
      rtlFmt.wrapWithUnicode(en, direction: TextDirection.LTR),
      equals('$LRE$en$PDF$RLM'),
    );
    // Test supposed overall dir (LTR) doesn't match context dir (RTL).
    expect(
      rtlFmt.wrapWithUnicode(he, direction: TextDirection.LTR),
      equals('$LRE$he$PDF$RLM'),
    );
    // Test supposed overall dir (LTR) doesn't match context dir (unknown).
    expect(
      unkFmt.wrapWithUnicode(he, direction: TextDirection.LTR),
      equals('$LRE$he$PDF'),
    );
    // Test supposed overall dir (neutral) doesn't match context dir (LTR).
    expect(
      ltrFmt.wrapWithUnicode(he, direction: TextDirection.UNKNOWN),
      equals('$he$LRM'),
    );
  });
}
