...
## Analyzer Plugin & Quick Fixes

`package:messages` includes an analysis server plugin (`messages_analyzer_plugin`) that flags hardcoded string literals (`literal_string_outside_l10n`) and provides an IDE **Extract to ARB** Quick Fix.

To enable the lint rule and Quick Fixes automatically in your project, add the following to your project's `analysis_options.yaml`:

```yaml
include: package:messages_builder/analysis_options.yaml
```
