// Copyright (c) 2023, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.

import 'dart:async';
import 'dart:convert';
import 'dart:io';

import 'package:collection/collection.dart';
import 'package:messages_serializer/messages_serializer.dart';
import 'package:path/path.dart' as path;

import 'arb_parser.dart';
import 'code_generation/classes_generation.dart';
import 'code_generation/code_generation.dart';
import 'generation_options.dart';
import 'located_message_file.dart';
import 'message_data_builder.dart';
import 'message_file.dart';

class MessageCallingCodeGenerator {
  final GenerationOptions options;
  final Map<String, String> mapping;

  MessageCallingCodeGenerator({required this.options, required this.mapping});

  Future<void> build() async {
    final messageFiles = await _parseMessageFiles();

    final families = messageFiles
        .groupListsBy((messageFile) => getParentFile(messageFiles, messageFile))
        .map(
          (key, value) => MapEntry(
            key,
            value.sortedBy((messageFile) => messageFile.locale),
          ),
        );

    var counter = 0;
    final familyFiles = <String?, List<String>>{};
    final suffix = options.assetLoadingStyle == TargetEnvironment.dart
        ? 'data'
        : 'empty';

    for (final MapEntry(key: parent, value: children) in families.entries) {
      final context = parent.file.context;
      familyFiles[context] = children.map((f) => f.path).toList();

      final dummyFilePaths = Map.fromEntries(
        children
            .map((e) => e.locale)
            .map((e) => MapEntry(e, [context, e, suffix].nonNulls.join('_'))),
      );

      final library = ClassesGeneration(
        options: options,
        context: context,
        parent: parent,
        children: children,
        emptyFiles: dummyFilePaths,
      ).generate();

      final code = CodeGenerator(
        options: options,
        classes: library,
        emptyFilePaths: dummyFilePaths.values,
      ).generate();

      final parentPath = Directory(options.codeOutputFolder.path);

      final mainFile = File(
        path.join(
          parentPath.path,
          '${context ?? 'm${counter++}'}_messages.g.dart',
        ),
      );
      await mainFile.create(recursive: true);
      await mainFile.writeAsString(code);

      for (final MapEntry(key: locale, value: emptyFilePath)
          in dummyFilePaths.entries) {
        final file = File(path.join(parentPath.path, '$emptyFilePath.g.dart'));
        await file.create(recursive: true);
        var content =
            '''
// Generated by package:messages_builder.
// Helper file for deferred loading of the messages for locale $locale.
''';
        if (options.assetLoadingStyle == TargetEnvironment.dart) {
          final childFile = children.firstWhere((c) => c.locale == locale);
          final dataString = arbToData(childFile.file, JsonSerializer());

          content +=
              '''
const String data = r\'\'\'$dataString\'\'\';
''';
        }
        await file.writeAsString(content);
      }
    }

    if (options.assetLoadingStyle != TargetEnvironment.dart) {
      printIncludeFilesNotification(familyFiles);
    }
  }

  Future<List<LocatedMessageFile>> _parseMessageFiles() async => Future.wait(
    mapping.entries
        .map(
          (p) async => LocatedMessageFile(
            path: path.relative(p.value, from: Directory.current.path),
            file: await parseMessageFile(await getArbfile(p.key), options),
          ),
        )
        .toList(),
  );

  Future<String> getArbfile(String path) async =>
      await File(path).readAsString();

  /// Either get the referenced parent file, or try to infer which it might be.
  static LocatedMessageFile getParentFile(
    List<LocatedMessageFile> messageFiles,
    LocatedMessageFile currentFile,
  ) {
    /// Try to infer by looking at which files contain metadata, which is a sign
    /// they might be the references for others in the same context.
    final filesInContext = messageFiles.where(
      (messageFile) => messageFile.file.context == currentFile.file.context,
    );
    final potentialParent = filesInContext.firstWhereOrNull(
      (element) => element.file.hasMetadata,
    );
    if (potentialParent == null && filesInContext.length > 1) {
      throw ArgumentError(
        '''
The files $filesInContext have no metadata, so it is not clear which one is the main source of truth.''',
      );
    }
    return potentialParent ?? currentFile;
  }

  /// Display a notification to the user to include the newly generated files
  /// in their assets.
  void printIncludeFilesNotification(Map<String?, List<String>> familyFiles) {
    if (familyFiles.isEmpty) return;
    print('Add the following to your pubspec.yaml:\n');
    print('flutter:');
    print('  assets:');
    for (final MapEntry(key: context, value: files) in familyFiles.entries) {
      if (context != null) {
        print('    # $context');
      }
      final sortedFiles = files.toList()..sort();
      for (final file in sortedFiles) {
        print('    - $file');
      }
    }
  }
}

Future<MessageFile> parseMessageFile(
  String arbFile,
  GenerationOptions options,
) async {
  final decoded = (jsonDecode(arbFile) as Map).cast<String, Object?>();
  return ArbParser().parseMessageFile(decoded);
}

/// Deletes old generated files (`.g.dart` in `codeOutputFolder` and
/// `.arb.json` in `messageFolder`).
Future<void> deleteOldGeneratedFiles(GenerationOptions options) async {
  final codeDir = options.codeOutputFolder;
  if (await codeDir.exists()) {
    await for (final entity in codeDir.list(recursive: true)) {
      if (entity is File && entity.path.endsWith('.g.dart')) {
        final content = await entity.readAsString();
        if (_isGeneratedFile(content)) {
          await entity.delete();
        }
      }
    }
  }

  final messageDir = options.messageFolder;
  if (await messageDir.exists()) {
    await for (final entity in messageDir.list(recursive: false)) {
      if (entity is File && entity.path.endsWith('.arb.json')) {
        await entity.delete();
      }
    }
  }
}

bool _isGeneratedFile(String content) {
  return content.contains('Generated by package:messages_builder.');
}
